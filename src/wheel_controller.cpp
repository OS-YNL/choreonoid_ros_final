...
class WheelController : public cnoid::SimpleController
{
public:
...
bool WheelController::initialize(cnoid::SimpleControllerIO* io)
{
    auto body = io->body();
    ...
    subscriber = node->subscribe(
        "/cmd_vel", 1, &WheelController::commandCallback, this);
    return true;
}

void WheelController::commandCallback(const geometry_msgs::Twist& twist)
{
    std::lock_guard<std::mutex> lock(commandMutex);
    command = twist;
}

bool WheelController::control()
{
    constexpr double wheelRadius = 0.076;
    constexpr double halfAxleWidth = 0.112;
...
CNOID_IMPLEMENT_SIMPLE_CONTROLLER_FACTORY(WheelController)